...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Existing LoRA checkpoint whose weights initialize this training stage.",
    )
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Use the complete input datasets. Otherwise use a 100/50 smoke test.",
    )
...
def create_dataset_preview(dataset: Path, output: Path, count: int) -> None:
    ...
    column_count = max(len(record.get("images", [])) for record in records)
    if column_count < 1:
        raise ValueError("Preview records do not contain image paths")
    figure, axes = plt.subplots(
        len(records),
        column_count,
        figsize=(5.5 * column_count, max(4, len(records) * 3.5)),
        squeeze=False,
    )
    for row, record in enumerate(records):
        image_paths = [Path(path) for path in record.get("images", [])]
        for path in image_paths:
            require_file(path, "Preview image")
        ...
        axes[row][0].axis("off")
        modality_titles = ["Visible RGB", "Thermal infrared", "Depth map"]
        for column in range(1, column_count):
            if column < len(opened_images):
                axes[row][column].imshow(opened_images[column])
                title = (
                    modality_titles[column]
                    if column < len(modality_titles)
                    else f"Image {column + 1}"
                )
    ...
    figure.suptitle("Training samples and RGB target boxes", fontsize=13)
    figure.tight_layout()
...
def main() -> int:
    ...
    adapters = args.adapters.resolve() if args.adapters is not None else None
    ...
    if adapters is not None:
        require_file(adapters / "adapter_config.json", "Adapter config")
        require_file(adapters / "adapter_model.safetensors", "Adapter weights")
